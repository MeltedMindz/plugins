 ...
 */
export function getOrCreateProject(projectPath) {
    let project = stmts.getProject.get(projectPath);
    if (!project) {
        // Extract name from path
        const name = projectPath.split('/').pop() || projectPath;
        const result = stmts.insertProject.run(projectPath, name);
        project = stmts.getProjectById.get(result.lastInsertRowid);
    }
    return project;
}
/**
...
export function addDecision(params) {
    ...
    const result = stmts.insertDecision.run(project.id, JSON.stringify(params.files), params.reasoning, params.commit || null, params.commitUrl || null, JSON.stringify(params.tags || []), params.context || null);
...
export function getDecision(id) {
    const row = stmts.getDecision.get(id);
    if (!row)
        return null;
    return {
        ...row,
 ...
 */
export function linkCommit(decisionId, commitHash, commitUrl) {
...
export function getRecentDecisions(limit = 20) {
    const rows = stmts.getRecentDecisions.all(limit);
    return rows.map(row => ({
        ...row,
        files: JSON.parse(row.files),
        tags: JSON.parse(row.tags),
 ...
 */
export function getProjectDecisions(projectPath, limit = 100) {
    const rows = stmts.getProjectDecisions.all(projectPath, limit);
    return rows.map(row => ({
        ...row,
        files: JSON.parse(row.files),
        tags: JSON.parse(row.tags),
 ...
 */
export function getFileDecisions(filePath) {
    const rows = stmts.getFileDecisions.all(`%${filePath}%`);
    return rows.map(row => ({
        ...row,
        files: JSON.parse(row.files),
        tags: JSON.parse(row.tags),
 ...
 */
export function searchDecisions(query, options) {
 ...
 */
export function countProjectDecisions(projectPath) {
    const result = stmts.countProjectDecisions.get(projectPath);
    return result.count;
}
/**
